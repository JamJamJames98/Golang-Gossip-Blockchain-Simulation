...
def printResultsAttributes(result, neighbourListPercentage):
	...
	print("For Node: ", neighbourListPercentage, "%")
...
def getIndex(file):
	...
	if (line == -1):
		return -1
	return int(line.partition("[START] ")[2].partition(" ")[0])

def getNumberOfNodes(file):
	line = getNewLine(file, False)
	return line.partition("Beginning Gossip for: ")[2].partition(" ")[0]

def confirmNeighbourListPercentage(file, index):
	line = getNewLine(file, False)
	confirmationPercentage = int(line.partition("NeighbourList Percentage: ")[2].partition(" ")[0])
	if (index != confirmationPercentage):
...
resultsList = [Results() for i in range(101)]
...
while True:
	...
	if (resultsList[index].totalNodes == -1):
		resultsList[index].totalNodes = getNumberOfNodes(file)
	else:
		if resultsList[index].totalNodes != getNumberOfNodes(file):
			print("Incompatible results file, use the same node size setup throughout")
			exit(1)

	confirmNeighbourListPercentage(file, index)
	...
	if (nodesReached == 0):
		resultsList[index].avgNodesNotReached += 100
	elif (nodesReached == 100):
